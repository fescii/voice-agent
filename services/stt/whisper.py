...
class WhisperService:
    ...
    async def transcribe_audio(
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Successfully transcribed audio: {len(result.get('text', ''))} characters")
                return result
    ...
    async def translate_audio(
        ...
        """
        try:
            logger.info(f"Translating audio ({len(audio_data)} bytes, format: {file_format})")
            ...
            try:
                ...
                logger.info(f"Successfully translated audio: {len(result.get('text', ''))} characters")
                return result
            ...
            logger.error(f"Whisper translation API error: {e.response.status_code} - {e.response.text}")
            raise
        except Exception as e:
    ...
    async def transcribe_streaming(
        ...
        try:
            ...
            logger.info(f"Transcribing streaming audio ({len(combined_audio)} bytes from {len(audio_chunks)} chunks)")
