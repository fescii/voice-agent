"""
WebSocket connection core class.
"""
...
from ..models.enums import ConnectionState, AudioFormat
...
class WebSocketConnection:
  ...
  async def authenticate(self, user_id: str) -> None:
    """Authenticate the connection with a user ID"""
    self.user_id = user_id
    self.state = ConnectionState.AUTHENTICATED
    self.last_activity = datetime.now(timezone.utc)
    logger.info(f"WebSocket authenticated for user: {user_id}")

  async def send_message(self, message_type: str, data: Any = None) -> None:
  ...
  def get_info(self) -> Dict[str, Any]:
    ...
    return {
        ...
        "call_context": self.call_context.call_id if self.call_context else None,
        "audio_format": self.audio_format.value,
