...
async def websocket_audio_endpoint(
  ...
  try:
    ...
    logger.error(
        f"Failed to establish WebSocket connection for call {call_id}: {str(e)}")
...
async def websocket_control_endpoint(
  ...
  try:
    ...
    logger.error(
        f"Failed to establish control WebSocket for call {call_id}: {str(e)}")
    await websocket.close(code=1011, reason="Internal server error")
...
async def _handle_control_message(call_id: str, message: Dict, websocket: WebSocket):
  ...
  if message_type == "ping":
    ...
    logger.info(
        f"Transfer request for call {call_id} to {message.get('target')}")
