...
class ToolDefinition(BaseModel):
    ...
    required: bool = Field(default=False, description="Whether this tool is required")
...
class EdgeCondition(BaseModel):
    ...
    type: str = Field(..., description="Type of condition (e.g., 'intent', 'entity', 'sentiment')")
...
class Edge(BaseModel):
    ...
    condition: Optional[EdgeCondition] = Field(None, description="Condition for this transition")
...
class State(BaseModel):
    ...
    tools: List[str] = Field(default_factory=list, description="Tools available in this state")
    description: Optional[str] = Field(None, description="Human-readable description")
...
class ScriptSection(BaseModel):
    ...
    weight: float = Field(default=1.0, description="Importance weight for token prioritization")


class ScriptSchema(BaseModel):
    """Schema for JSON-based prompt script."""
    name: str = Field(..., description="Name of the script")
    description: Optional[str] = Field(None, description="Description of the script's purpose")
    version: str = Field(default="1.0", description="Script version")
    general_prompt: Optional[str] = Field(None, description="General system prompt")
    sections: List[ScriptSection] = Field(default_factory=list, description="Script sections")
    states: List[State] = Field(default_factory=list, description="Conversation states")
    edges: List[Edge] = Field(default_factory=list, description="State transitions")
    tools: List[ToolDefinition] = Field(default_factory=list, description="Available tools")
    general_tools: List[str] = Field(default_factory=list, description="Tools available in all states")
    starting_state: Optional[str] = Field(None, description="Initial state name")
    dynamic_variables: Dict[str, str] = Field(default_factory=dict, description="Default variable values")
