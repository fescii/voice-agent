...
class OutboundCallService:
    ...
    async def initiate_call(
        ...
        try:
            ...
            logger.info(f"Initiating outbound call {call_id} to {phone_number} with agent {agent_id}")
            ...
            if not ringover_response.success:
                logger.error(f"Ringover call initiation failed: {ringover_response.error}")
            ...
            logger.info(f"Successfully initiated call {call_id} via Ringover (ID: {ringover_response.ringover_call_id})")
    ...
    async def schedule_call(
        ...
        logger.info(f"Scheduling outbound call {call_id} to {phone_number} for {scheduled_time}")
