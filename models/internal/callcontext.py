...
class CallContext(BaseModel):
    ...
    phone_number: str = Field(..., description="Phone number involved in the call")
    agent_id: str = Field(..., description="ID of the agent handling the call")
    direction: CallDirection = Field(..., description="Call direction (inbound/outbound)")
    status: CallStatus = Field(..., description="Current status of the call")
    start_time: Optional[datetime] = Field(None, description="When the call started")
    ...
    ringover_call_id: Optional[str] = Field(None, description="Ringover's internal call ID")
    websocket_id: Optional[str] = Field(None, description="WebSocket connection ID for audio")
...
class AudioChunk(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="When the audio was captured")
...
class TranscriptionResult(BaseModel):
    ...
    call_id: str = Field(..., description="Call ID this transcription belongs to")
    text: str = Field(..., description="Transcribed text")
    confidence: float = Field(..., description="Transcription confidence score")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="When the transcription was generated")
